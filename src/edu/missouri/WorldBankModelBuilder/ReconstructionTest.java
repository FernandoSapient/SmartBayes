...
public class ReconstructionTest {
	...
	public static Map<String, DoubleSummaryStatistics> crossValidationAccuracies(
			Map<Instances, Instances> splits, String filename, int values)
		...
		while (trains.hasNext()) {
			...
			boolean fileCreated = constructUnescoToFile(training, filename,
					values);
			assert fileCreated;
	 ...
	 *            Bayesian network to train; at {@code args[2]} the path of
	 *            the output file in which to store the trained network; and
	 *            optionally, at {@code args[3]} the index with the values
	 *            to group the results by (zero by default); at {@code args[4]}
	 *            the number of bins to use in discretization, and at 
	 *            {@code args[5]}, "true" if frequency discrtization is desired.
	 * @throws Exception
	...
	public static void main(String[] args) throws Exception {
		if (args.length < 3) {
			System.err.println("Usage: java RecontructionTest <input data file> <output directory> <working directory> <group-by column> [number of discrete values] [use equal frequency]");
			return;
		}
		...
		int values;
		if (args.length >= 4)
			values = Integer.parseInt(args[4]);
		else
			values = 3;
		boolean useEqualFrequency;
		if (args.length >= 5)
			useEqualFrequency = Boolean.getBoolean(args[4]);
		else
		...
		maxCSV.close();
		avgCSV.close();
		List<String> titles = Arrays.asList(getUnescoTitles());
		...
		while (C.hasNext()) {
			...
			boolean fileCreated = constructEmptyUnescoToFile(countryData,
					BN_File, values);
			if (fileCreated)
			...
			try {
				countryData = Trainer.conformToNetwork(countryData,
						wekaBayes, useEqualFrequency);
				...
				minCSV = new CSVWriter(new FileWriter(outFile+"/min.csv", true),',',CSVWriter.NO_QUOTE_CHARACTER);
				maxCSV = new CSVWriter(new FileWriter(outFile+"/max.csv", true),',',CSVWriter.NO_QUOTE_CHARACTER);
				avgCSV = new CSVWriter(new FileWriter(outFile+"/avg.csv", true),',',CSVWriter.NO_QUOTE_CHARACTER);
				mins[0]=country;
				maxs[0]=country;
				avgs[0]=country;
